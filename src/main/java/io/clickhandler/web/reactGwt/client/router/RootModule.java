...
public abstract class RootModule extends ModuleLoader {
    private final io.clickhandler.web.reactGwt.client.router.RouteComponent root;

    @Inject
    RootRoute rootRoute;

    @Inject
    Provider<io.clickhandler.web.reactGwt.client.router.RouteGatekeeper> routeGatekeeper;

    private Route appRoute;

    public RootModule(io.clickhandler.web.reactGwt.client.router.RouteComponent root) {
        super("/");
        this.root = root;
    }

    protected io.clickhandler.web.reactGwt.client.router.History history() {
