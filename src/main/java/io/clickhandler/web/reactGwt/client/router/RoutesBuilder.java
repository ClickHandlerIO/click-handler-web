...
public abstract class RoutesBuilder {
    ...
    protected void registerComponents() {
        ...
        Reflection.iterate(this, (name, value) -> {
            add((io.clickhandler.web.reactGwt.client.router.RouteComponent) value);
        });
    }

    protected <R extends io.clickhandler.web.reactGwt.client.router.RouteProxy<A>, A, P extends io.clickhandler.web.reactGwt.client.router.RouteProps, S> void add(io.clickhandler.web.reactGwt.client.router.RouteComponent<R, A, P, S> component) {
        // Instantiate RouteProxy.
        final io.clickhandler.web.reactGwt.client.router.RouteProxy<A> proxy = component.getRouteProxyProvider().get();
    ...
    void buildRoutes() {
        ...
        for (Reg reg : regs.values()) {
            ...
            io.clickhandler.web.reactGwt.client.router.RouteProxy parent = reg.proxy.parent();
    ...
    private final static class Reg {
        ...
        private final io.clickhandler.web.reactGwt.client.router.RouteProxy<?> proxy;
        private List<Reg> children = null;

        public Reg(Route route, io.clickhandler.web.reactGwt.client.router.RouteProxy<?> proxy) {
            this.route = route;
            this.proxy = proxy;
        }
